...
2. **Réseau de neurones 3D (Three.js r134)** — 7 couches `[1, 6, 9, 12, 9, 6, 1]`, chaque couche est une grille 2D (rows × cols) dans Y×Z pour une structure cubique.
   ...
   - **Pool dynamique de 18 particules** (`particlePool`) — chaque particule choisit une connexion aléatoire après chaque traversée, avec vitesse et délai de pause aléatoires. Plus de particules fixes sur des edges statiques.
   - Connexions : opacité de base `weight × 0.28` (visible au repos). Quand une particule traverse, activation progressive via bell curve `sin(t×π)` — montée douce, pic au milieu, déclin lent. Couleur glisse violet → cyan selon activation.
   - Rotation : vitesse angulaire (`neuralVelY`) pilotée par la souris, friction 0.93, rotation libre 360°
   - Clic "Découvrir mon profil" : shockwave (150 frames, bell-curve `sin^0.12` = plateau long ~75% de la durée), scroll déclenché après 1200ms.

3. **Hero background (2D canvas)** — constellation de 110 particules avec :
   - 3 ambient glow blobs qui dérivent lentement (radial gradients cyan/indigo)
   - Répulsion douce des particules autour du curseur (rayon 110px)
   - Scintillement subtil (amplitude 0.12)
   - ~~Réticule de scan~~ supprimé

4. **Background stars (2D canvas fixe)** — ~100 étoiles twinkling cyan-violet sur toute la page, alpha max 0.55.

5. **Curseur personnalisé** — canvas overlay fixe (`z-index: 99999`, `pointer-events: none`). Curseur système conservé. Aura radiale douce cyan/indigo qui suit la souris. S'estompe à 0% d'opacité quand le curseur survole un élément de contenu (détection via `document.elementFromPoint`).
